...
# download conceptnet 5.7: https://s3.amazonaws.com/conceptnet/downloads/2019/edges/conceptnet-assertions-5.7.0.csv.gz
...
def extract_english():
    ...
    config = configparser.ConfigParser()
    config.read("paths.cfg")
    fw = open(config["paths"]["conceptnet_en"], "w", encoding="utf8")
    with open(config["paths"]["conceptnet"], encoding="utf8") as f:
        ...
        while line:
            ...
            if ls[2].startswith('/c/en/') and ls[3].startswith('/c/en/'):
                ...
                fw.write("\t".join([rel, head, tail, str(data["weight"])]))
                fw.write("\n")
